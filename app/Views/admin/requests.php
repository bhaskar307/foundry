<div class="content-body p-3">
        ...
        <div class="d-flex align-items-center p-3 gap-4">
            ...
            <div class="filterdropdown filterfield">
                <div class="input-group">
                    <select class="form-select" id="search_vendor" aria-label="Default select example">
                        <option value=''>All Vendor</option>
                        <?php if (!empty($vendor)) {
                            foreach ($vendor as $key) {
                        ?>
                                <option value='<?= $key['uid'] ?>' <?php if ($key['uid'] == $vendorUid) { ?> selected <?php } ?>><?= $key['name'] ?></option>
                        <?php }
                        } ?>
                    </select>
                </div>
    ...
    <div class="mt-4 rounded-10 bg-white border">
        ...
        <div class="px-3 pb-3">
            <table id="tableRequest" class=" display border">
                <thead>
                    <tr>
                        <th>Customer Name</th>
                        <th>Product Name</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function handleFilterChange() {
                const vendor = $('#search_vendor').val();
                const customer = $('#search_customer').val();
                const product = $('#search_product').val();
                const date = $('#search_date').val();

                window.location.href = `<?= base_url('admin/requests') ?>?vendor=${vendor}&customer=${customer}&product=${product}&date=${date}`;

            }

            $('#search_vendor').on('change', handleFilterChange);
