    ...
    <?php } ?>
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton1">
                ...
                <li><a class="dropdown-item" href="<?= base_url('clinic/logout') ?>">Logout</a></li>
            </ul>
        </div>
    </div>
</div>
...
<script>
const base_url_nav = "<?= base_url(); ?>";
const assets = "assets/";
let apiKey_nav = localStorage.getItem("apiKey");
let clinicUid_nav = localStorage.getItem("clinicUid");

document.addEventListener("DOMContentLoaded", function () {
    fetch(base_url_nav + "api/clinic/detailsNav?clinicUid=" + encodeURIComponent(clinicUid_nav), {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
            "X-API-KEY": apiKey_nav 
        }
    })
    ...
    .then(data => {
        ...
        // Set name
        const nameElement = document.querySelector(".d-md-block h6");
        console.log(clinic.name);
        if (nameElement && clinic.name) {
            nameElement.textContent = clinic.name;
        }

        const welcomeElement = document.querySelector(".welcome-message");
        if (welcomeElement && clinic.name) {
            welcomeElement.textContent += ` ${clinic.name}`;
        }

        // Set image
        const imgElement = document.querySelector("img.rounded-circle");
        if (imgElement) {
            const imagePath = clinic.logo && clinic.logo.trim() !== ""
                ? base_url_nav + 'assets/' + clinic.logo.replace("//", "/") // Clean path
                : base_url_nav + "assets/clinic/images/no_image.png"; // fallback
